...
def upgrade() -> None:
    ...
    op.create_foreign_key("prediction_class_prediction_fkey", "prediction", "prediction_class", ["predicted_class_id"], ["id"])
    ...
    if len(res) == 0:
        op.bulk_insert(
            models_table,
            [
                {"id": id_, "name": name, "description": description, "cost": cost}
                for id_, name, description, cost in models
            ],
        )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "prediction", sa.Column("output", postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True)  # type: ignore[no-untyped-call]
    )
    op.add_column(
        "prediction", sa.Column("input", postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True)  # type: ignore[no-untyped-call]
    )
